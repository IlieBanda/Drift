...
rm -rf "$APP_BUNDLE"; mkdir -p "$APP_MACOS" "$APP_RESOURCES"; cp "$BUILD_BINARY" "$APP_BINARY"; cp "$ROOT_DIR/Resources/Swarm.icns" "$APP_RESOURCES/Swarm.icns"; chmod +x "$APP_BINARY"
...
codesign --force --deep --sign "${DEVELOPER_ID_IDENTITY:--}" --identifier "$BUNDLE_ID" --options runtime --entitlements "$ROOT_DIR/Swarm.entitlements" "$APP_BUNDLE"
case "$MODE" in
